...
class APIConnection:
  class Lesson:
    def __init__(self, period: PeriodObject) -> None:
      ...
      self.exam = (hasattr(period, 'info') and any(x in period.info.lower() for x in ['prüf', 'klausur', 'exam', 'lek', 'test'])) or self.activityType == 'ex' # type: ignore
    ...
    def __str__(self) -> str:
      ...
      return f'{'X' if cancelled else 'O'};{start}-+-{end} | cla={str(klassen):^10}, sub={str(subjects):^15}, stu={students:^20}, room={str(rooms):^15}'
    def __repr__(self) -> str:
      return self.__str__()

  def __init__(self, surname: Optional[str], forename: Optional[str], username: Optional[str], password: Optional[str], school: Optional[str]) -> None:
